...
import argparse
import sys
from indexing import initialize_components, create_storage_context_and_index, load_chunks_from_jsonl, add_nodes_to_index
from query import load_existing_index, create_query_engine, query_book, get_chunks_by_position, get_current_position_context, summarize_chunk_range
from llama_index.core import Settings

CHUNKS_PER_PAGE = 10  # Number of chunks that make up one "page"
...
# Extract, Parse, Clean, Chunk, and Enrich with Metadata
def process_epub_to_chunks(book, book_title=None, author=None):
    """
    Process EPUB and create chunks with metadata for RAG system.
    Each chunk represents a paragraph with its contextual metadata.
    """
...
def write_chunks_to_jsonl(chunks, output_filename='book_chunks.jsonl'):
    """
...
def setup_system(book_path, chunks_file, chroma_db_path, model_name):
    """
    ...
    if not os.path.exists(chunks_file):
        ...
        chunks = process_epub_to_chunks(book)
        print(f"Created {len(chunks)} chunks")
        
        print("Writing chunks to JSONL...")
        write_chunks_to_jsonl(chunks, chunks_file)
    ...
    if not os.path.exists(chroma_db_path):
        ...
        print("Loading chunks and creating nodes...")
        nodes = load_chunks_from_jsonl(chunks_file)
...
def handle_question(query_engine):
    ...
    print("\nProcessing...")
    print("1. RETRIEVE: Finding relevant chunks...")
    ...
    if hasattr(response, 'source_nodes') and response.source_nodes:
        ...
        for i, node in enumerate(response.source_nodes, 1):
            ...
            print(f"  Position: {node.metadata.get('chunk_position', 'N/A')}")
            print(f"  Preview: {node.text[:150]}...")
    else:
        print("\nNo source chunks were retrieved. This might indicate:")
...
def handle_range_summary(chunks_file):
    """
    ...
    try:
        ...
        # Convert pages to chunk positions
        start_chunk = start_page * CHUNKS_PER_PAGE
        end_chunk = (end_page + 1) * CHUNKS_PER_PAGE - 1
        
        print(f"\nRetrieving pages {start_page} to {end_page} (chunks {start_chunk}-{end_chunk})...")
        chunks = get_chunks_by_position(chunks_file, start_chunk, end_chunk)
        
        if not chunks:
            print("No pages found in that range.")
            return
        
        print(f"Found {len(chunks)} chunks across {end_page - start_page + 1} pages")
        print("\nGenerating summary...")
        
        summary = summarize_chunk_range(Settings.llm, chunks)
...
def handle_context_summary(chunks_file):
    """
    ...
    try:
        ...
        print(f"  (Pages {start_page}-{end_page}, chunks {start_chunk}-{end_chunk})")
        chunks = get_chunks_by_position(chunks_file, start_chunk, end_chunk)
        ...
        print(f"\nFound {len(chunks)} chunks across {num_pages} pages")
        
        # Group chunks by page for display
        pages = {}
        for chunk in chunks:
            chunk_pos = chunk.get('id', chunk['metadata'].get('chunk_position', 'N/A'))
            page_num = chunk_pos // CHUNKS_PER_PAGE
        ...
        for page_num in sorted(pages.keys()):
            ...
            chapters = set(c['metadata'].get('chapter') for c in page_chunks if c['metadata'].get('chapter'))
            chapter_str = ', '.join(chapters) if chapters else 'Unknown'
            print(f"  - Page {page_num}: {len(page_chunks)} chunks ({chapter_str})")
...
Models (sorted by speed):
  ...
  qwen2.5:3b       - Best quality, 2GB, 4GB RAM (slower)
    ...
    parser.add_argument('-o', '--output-dir', type=str, default='.',
                        help='Output directory for chunks and database (default: current directory)')
    parser.add_argument('-k', '--top-k', type=int, default=5,
                        help='Number of chunks to retrieve for each query (default: 5)')
...
def main():
    ...
    chunks_file = os.path.join(args.output_dir, f"{book_name}_chunks.jsonl")
    ...
    setup_system(args.book, chunks_file, chroma_db_path, args.model)
    ...
    while True:
        ...
        if choice == '1':
            ...
            handle_range_summary(chunks_file)
        elif choice == '3':
            handle_context_summary(chunks_file)
        elif choice == '4':
            print("\nGoodbye!")
            break
        else:
